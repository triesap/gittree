...
fn load_runtime_config() -> Result<SyncConfig, SyncError> {
    Ok(runtime_config())
}

fn load_config_error() -> Result<SyncConfig, SyncError> {
    Err(SyncError::Serve("config failed".to_string()))
}

fn serve_ok(_: SyncConfig) -> super::MainRunFuture {
    Box::pin(async { Ok::<(), SyncError>(()) })
}

fn serve_err(_: SyncConfig) -> super::MainRunFuture {
    Box::pin(async { Err::<(), SyncError>(SyncError::Serve("serve failed".to_string())) })
}
...
async fn main_impl_with_reports_errors() {
    ...
    let mut run_fn = || -> super::MainRunFuture {
        Box::pin(async { Err::<(), SyncError>(SyncError::Serve("boom".to_string())) })
    };
...
async fn main_impl_with_executes_loader() {
    ...
    let mut run_fn = || -> super::MainRunFuture { Box::pin(async { Ok::<(), SyncError>(()) }) };
